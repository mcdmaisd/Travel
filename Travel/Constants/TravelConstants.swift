//
//  TravelConstants.swift
//  Travel
//
//  Created by ilim on 2025-01-06.
//

import Foundation

struct TravelConstants {
    static let cornerRadius: CGFloat = 10
    static let boldSize: CGFloat = 20
    static let systemfontSize: CGFloat = 16
    static let numberOfLines = 0
    static let cityCellEstimatedHeight: CGFloat = 100
    static let InfoCellEstimatedHeight: CGFloat = 500
    static let shoppingCellHeight: CGFloat = 50
    static let shoppingSectionHeight: CGFloat = 30
    static let normalLikeButton = "heart"
    static let selectedLikeButton = "heart.fill"
    static let imageNames = ["checkmark.square", "star"]
    static let selectedSuffix = ".fill"
    static let stringFormat = "yyMMdd"
    static let dateStringFormat = "yy년 MM월 dd일"
    static let chatDateInputForamt = "yyyy.MM.dd HH:mm"
    static let chatDateOutputForamt = "yy.MM.dd"
    static let arrayUserDefaultsKey = "shoppingList"
    static let todoPlacdHolder = "무엇을 구매하실 건가요?"
    static let minimumCharacterMessage = "최소 1자 이상 입력해 주세요"
    static let sectionNames = ["진행중", "완료됨"]
    static let segmentTitles = ["모두", "국내", "해외"]
    static let categoryName = "Hangul"
    static let separator = " | "
    static let cancelImage = "xmark"
    static let backImage = "chevron.backward"
}
